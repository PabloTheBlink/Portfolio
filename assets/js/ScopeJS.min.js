...
export function Component({tagName,controller,render,postRender}){const c=new(function Component(){this.render=function(container=document.createElement("div"),children=[]){const uuid=container.getAttribute(UUID_ATTRIBUTE)??crypto.randomUUID();function updateDomChildren(container,clone){if(container.getAttribute(UUID_ATTRIBUTE)&&container.getAttribute(UUID_ATTRIBUTE)!=uuid)return;const originalChildren=container.childNodes;const updatedChildren=clone.childNodes;const maxLength=Math.max(originalChildren.length,updatedChildren.length);let style=document.querySelector("style[scopejs]");if(!style){style=document.createElement("style");style.setAttribute("scopejs","1");style.innerHTML=`::view-transition-old(*), ::view-transition-new(*) { animation-timing-function: ease-in-out; animation-duration: 0.25s; }`;document.head.appendChild(style)}
...
const apply=function(){if(!container)return;const clone=container.cloneNode(!0);clone.innerHTML=render.bind(c)();updateDomChildren(container,clone);container.querySelectorAll("*[model]").forEach((el)=>{const modelName=el.getAttribute("model");const spl=modelName.split(".");let value=c;for(let item of spl){if(!value)continue;value=value[item]}
el.value=value;el.addEventListener("input",()=>{let currentValue=c;for(let i=0;i<spl.length-1;i++){currentValue=currentValue[spl[i]]}
...
if(!route){this.alias=params.error?(params.error.alias.startsWith(":")?this.params[params.error.alias.split(":")[1]]:params.error.alias):"404";this.current_component=Component(params.error?params.error.controller:{render:()=>"404"}).render(this.container)}else{this.alias=route.alias.startsWith(":")?this.params[route.alias.split(":")[1]]:route.alias;if(this.current_component)destroyRecursive(this.current_component);this.current_component=Component(route.controller).render(this.container)}
for(let listener in this.listeners)this.listeners[listener](this.params);};window.addEventListener("popstate",(e)=>{this.render()})})()}
