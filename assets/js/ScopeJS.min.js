...
const apply=function(){if(!container)return;const rendered=render.bind(c)();if(!rendered)return;c._render_times++;const clone=container.cloneNode(!0);clone.innerHTML=rendered;updateDomChildren(container,clone);container.querySelectorAll("*[model]").forEach((el)=>{const modelName=el.getAttribute("model");const spl=modelName.split(".");let value=c;for(let item of spl){if(!value)continue;value=value[item]}
